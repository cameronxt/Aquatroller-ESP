...
struct Channel {
  ...
  //  Following variables are in because max vaue exceed byte
  ...
  unsigned long moonlightFadeDelay = 2000; // fade delay when turning on moonlights Default: 2000 = 2 seconds
  int stepCount;
  int currentBright;
...
class Light {
  ...
  public:
    ...
    void init();
    void loop(time_t currentTime);

    LightData * getDataAddress() {
      return &_data;
    };

    void normalLights();
    void customLights(unsigned long currentMillis);
    void testLights();
    void moonLights();
