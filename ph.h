 ...
 * and is the most recently calculated reading. Using this PH reading we can then start 
 ...
 * 120v(mains) solenoid on the c02 regulator. There are 12V solenoids that could be 
 * controlled through a transistor as well, dont forget the flyback diode though!
 * 
 ...
 *     example:
        PH ph(A0, 8, &rtc);
        float phNow = ph._currentPh; // Note the ph. infront of variables and class functions
 * - Option 2: Modify or rewrite my loop(). Downside to this is you will lose your 
 ...
 * TODO: Bypass c02 build-up routine if we start in he middle of an on period
...
struct PHData {
  ...
  unsigned long checkC02Delay = 30UL * 1000;  // Delay before cycling c02 relay in millis seconds

  unsigned long checkPhDelay = 5000UL;         // Wait one second between Checks, this allows sensor to stabilize // TODO: Change to seconds
  unsigned long phStabilizeDelay = 60UL * 60 * 2;  // Delay to wait for ph to fully stabilize                            // TODO: Change to seconds
  unsigned long c02OnTime = 12UL * 60 * 60;        // c02 On time in seconds since midnight                                    // TODO: Change to minutes
  unsigned long c02OffTime = 22UL * 60 * 60;       // c02 Offtime  in seconds since midnight                                                            // TODO: Change to minutes
  ...
  unsigned long c02OffgasDelay = 60UL * 60 * 2;  // Offgas delay, starts at 2 hours, plan to learn this based off PH readings - 2 hours
  unsigned long c02BuildupTime = 60UL * 30;      // How long before lights should we turn on C02 - 30 minutes
...
public:
  ...
  void setTargetPh(float newTarget) {
    _phData.targetPh = newTarget;
  };
  float getTargetPh() {
    return _phData.targetPh;
  };

  void setPhDelay(unsigned int newDelay) {
    _phData.checkPhDelay = newDelay;
  };
  unsigned int getPhDelay() {
    return _phData.checkPhDelay;
  };

  void setC02Delay(unsigned int newDelay) {
    _phData.checkC02Delay = newDelay;
  };
  unsigned int getC02Delay() {
    return _phData.checkC02Delay;
  };


  void setCalPoints();  // Set the calibration points
  float calculateCalibration() {
    _phData.phCalValue = ((_calTarget[0] - _calActual[0]) + (_calTarget[1] - _calActual[1])) / 2;
  };
  void setCalTarget(int index, float calTarget) {
    _calTarget[index] = calTarget;
  };
  void setCalActual(int index, float calActual) {
    _calActual[index] = calActual;
  };
  ...
  float getRestingPh() {
    return _phData.restingPh;
  };
  void setRestingPh(float restingPh) {
    _phData.restingPh = restingPh;
  };



  float getC02PhTarget() {
    return _phData.targetPhC02;
  };
  void setC02PhTarget(float c02PhTarget) {
    _phData.targetPhC02 = c02PhTarget;
  };

  float getTargetPhDrop() {
    return _phData.targetPhC02;
  };
  void setTargetPhDrop(float targetPhC02) {
    _phData.targetPhC02 = targetPhC02;
  };

  int getTargetPPMC02() {
    return _phData.targetPPMC02;
  };
  void setTargetPPMC02(int targetPPM) {
    _phData.targetPPMC02 = targetPPM;
  };

  float getKhHardness() {
    return _phData.khHardness;
  };
  void setKhHardness(float khHardness) {
    _phData.khHardness = khHardness;
  };

  unsigned int getC02OnTime() {
    return _phData.c02OnTime;
  };
  void setC02OnTime(unsigned int onTime) {
    _phData.c02OnTime;
  };

  unsigned int getC02OffTime() {
    return _phData.c02OffTime;
  };
  void setC02OffTime(unsigned int onTime) {
    _phData.c02OffTime;
  };
...
private:
  ...
  volatile bool _newPh = false;          // Flag to tell when we have a new ph reading
  ...
  volatile bool _co2On = false;         // Flag to tell state of c02
  float _previousPh = 0.0F;     // Only print changes in ph
  bool _prevC02Position = false;  // only print changes of valve
  bool _bufferFull = false;   // is buffer full
  // int* dataPtr;
};
